...
class DAIRIDatasetSeg(data.Dataset):
    def __init__(self, ida_aug_conf, bda_aug_conf, classes, image_path, label_path, filenames_list, seg_path=None, height_path=None, calib_path=None, training=True, 
        ...
        if True:
            ...
            if seg_path is not None:
                if seg_path[-1]!='/':
                    seg_path = seg_path + '/'
                self.seg_path = [seg_path + name + '.jpg' for name in filenames_list]
            else:
                self.seg_path = None
    ...
    def __getitem__(self, index):
        ...
        if self.seg_path is not None:
            cur_seg_path = self.seg_path[index]
        ...
        input_list = self.format_input(cur_image_path, cur_seg_path, cur_height_path, cur_calib_path)
        sweep_imgs, sweep_sensor2ego_mats, sweep_intrins, sweep_ida_mats, gt_segs, gt_heights, sweep_imgs_tb = input_list
        ...
        ret_list = [sweep_imgs, sweep_sensor2ego_mats, sweep_intrins, sweep_ida_mats, bda_mat, gt_boxes, gt_labels, gt_segs, gt_heights, sweep_imgs_tb, img_metas]
        return ret_list
    ...
    def format_input(self, image_path, seg_path, height_path, calib_path):
        sweep_imgs = list()
        sweep_imgs_tb = list()
        ...
        img_seg = img_seg.convert('L')
        img_seg, _ = img_transform(img_seg, resize=resize, resize_dims=resize_dims, crop=crop, flip=flip, rotate=rotate_ida)
        img_seg = torch.Tensor(np.array(img_seg))
        gt_seg.append(img_seg)
