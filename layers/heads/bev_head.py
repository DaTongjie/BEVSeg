...
class BEVHead(CenterHead):
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, has_velocity=False):
    ...
    def loss(self, targets, preds_dicts, **kwargs):
        ...
        return return_loss, loss_bbox, loss_heatmap
        ...
        for i in range(num_samples):
            for k in rets[0][i].keys():
                if k == 'bboxes':
                    ...
                    pad = torch.zeros(bboxes.shape[0],2).cuda()
                    bboxes = torch.cat((bboxes, pad),dim=1)
