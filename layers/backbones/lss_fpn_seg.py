...
class ResidualSELayer(nn.Module):
    ...
    def forward(self, x, x_se):
        ...
        return x + self.bn1(x) * self.gate(x_se)
...
class HeightNet(nn.Module):
    def __init__(self, in_channels, mid_channels, context_channels,
                 height_channels, use_residual_depth_se=False):
        super(HeightNet, self).__init__()
        self.use_residual_depth_se = use_residual_depth_se
        ...
        if self.use_residual_depth_se:
            self.height_se = ResidualSELayer(mid_channels)  # NOTE: add camera-aware
            self.context_se = ResidualSELayer(mid_channels)  # NOTE: add camera-aware
            self.seg_se = ResidualSELayer(mid_channels)        # NOTE: add camera-aware
        else:
    ...
    def forward(self, x, mats_dict):
        ...
        return torch.cat([height, seg, context], dim=1)


class LSSFPN_seg(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, h_bound, final_dim,
                 downsample_factor, output_channels, img_backbone_conf,
                 img_neck_conf, height_net_conf, softmax_height_pred=True):
        ...
        Args:
            ...
            height_net_conf (dict): Config for depth net.
        """

        super(LSSFPN_seg, self).__init__()
        self.softmax_height_pred = softmax_height_pred
    ...
    def create_frustum(self):
        ...
        # alpha = 1.5
        h_coords = np.arange(self.h_bound[2]) / self.h_bound[2]
        # d_coords = np.power(d_coords, alpha)
        h_coords = self.h_bound[0] + h_coords * (self.h_bound[1] - self.h_bound[0])
        h_coords = torch.tensor(h_coords, dtype=torch.float).view(-1, 1, 1).expand(-1, fH, fW)

        H, _, _ = h_coords.shape
        x_coords = torch.linspace(0, ogfW - 1, fW, dtype=torch.float).view(
            1, 1, fW).expand(H, fH, fW)
        y_coords = torch.linspace(0, ogfH - 1, fH,
                                  dtype=torch.float).view(1, fH,
                                                          1).expand(H, fH, fW)
        paddings = torch.ones_like(h_coords)

        # D x H x W x 3
        frustum = torch.stack((x_coords, y_coords, h_coords, paddings), -1)
        return frustum
    ...
    def get_geometry(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat):
        ...
        S = ((MAT2[:,:,:,:,:,2,0]+points[:,:,:,:,:,2,0])/MAT1[:,:,:,:,:,2,0]).view(batch_size,num_cams,coods.shape[2],coods.shape[3],coods.shape[4],1,1)
        points[:,:,:,:,:,:2,:] = MAT1[:,:,:,:,:,:2,:]*S - MAT2[:,:,:,:,:,:2,:]
    ...
    def _forward_single_sweep(self,
        ...
        height = height_feature[:, :self.height_channels]
        seg = height_feature[:, self.height_channels:self.height_channels + 1]
        if self.softmax_height_pred:
            height = height.softmax(1)
            #return height  # TODO:
            img_feat_with_height = height.unsqueeze(
                1) * height_feature[:, self.height_channels:].unsqueeze(2)
        else:
            img_feat_with_height = height.softmax(1).unsqueeze(
                1) * height_feature[:, self.height_channels:].unsqueeze(2)
        ...
        return feature_map.contiguous(), height, seg
